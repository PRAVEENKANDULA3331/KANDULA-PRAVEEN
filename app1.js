...
const DEFAULT_COMPANY_NAME="Dodla Dairy Limited";
const DEFAULT_COMPANY_LOGO="https://cdn.brandfetch.io/id5GmlLS2a/w/1448/h/1448/theme/dark/icon.jpeg?c=1bxid64Mup7aczewSAYMX&t=1768020612340";
let db={settings:{agency_name:"GANTALAMMA MILK AGENCY",company_name:DEFAULT_COMPANY_NAME,company_logo:DEFAULT_COMPANY_LOGO,whatsapp:"9573916154",upi_id:"9573916154@ybl",contact:"9908679198",credit_days:3},products:[],customers:[],prices:[],orders:[],items:[],payments:[]};
let currentPage="home", pollTimer=null, syncing=false, brandingLoaded=false;
...
function companyBrand(){let logo=s().company_logo||DEFAULT_COMPANY_LOGO,name=s().company_name||DEFAULT_COMPANY_NAME,agency=s().agency_name||"GANTALAMMA MILK AGENCY";return `<div class="company-brand">${logo?`<img class="company-logo" src="${esc(logo)}" alt="${esc(name)} logo" onerror="this.style.display='none'">`:""}<div><div class="company-name">${esc(name)}</div><div class="brand agency-brand">${esc(agency)}</div></div></div>`}
...
async function loadPublicBranding(){
 ...
 brandingLoaded=true;
 try{
  let b=await rpc("app_public_branding",{});
  if(b)db.settings={...db.settings,...b};
  document.title=`${s().company_name||DEFAULT_COMPANY_NAME} | ${s().agency_name||"GANTALAMMA MILK AGENCY"}`;
 }catch{}
}
...
function normalize(x){
 x=x||{}; return {
  settings:{...db.settings,...(x.settings||{})},
  products:x.products||[],
...
function shell(body,kind){
 return `<div class="top"><div class="wrap between"><div>${companyBrand()}<div class="sub portal-sub">${kind==="admin"?"ADMIN / DISTRIBUTOR":"CUSTOMER / SHOP OWNER"} • <span class="sync">Shared Supabase database</span></div></div><button class="btn ghost" onclick="logout()">Logout</button></div></div><div class="wrap">${body}</div><div class="footer"><b>${esc(s().company_name||DEFAULT_COMPANY_NAME)}</b> • ${esc(s().agency_name||"GANTALAMMA MILK AGENCY")} • ${esc(s().contact||"")}</div>`;
}
...
async function render(){
 ...
 document.getElementById("app").innerHTML=`<div class="top"><div class="wrap">${companyBrand()}<div class="sub portal-sub">Shared online ordering & accounts</div></div></div><div class="wrap login"><div class="card"><h3>Select Portal</h3><div class="grid"><button class="btn primary" onclick="location.href='?view=admin'">Admin Login</button><button class="btn green" onclick="location.href='?view=customer'">Customer Login / Register</button></div></div></div><div class="footer"><b>${esc(s().company_name||DEFAULT_COMPANY_NAME)}</b> • ${esc(s().agency_name||"GANTALAMMA MILK AGENCY")}</div>`;
}

function adminLogin(){
 app.innerHTML=`<div class="top"><div class="wrap">${companyBrand()}<div class="sub portal-sub">Admin Login</div></div></div><div class="wrap login"><div class="card"><label>Mobile / User ID</label><input id="au" value="9573916154"><label>Password</label><input id="ap" type="password"><button class="btn primary block" style="margin-top:12px" onclick="doAdminLogin()">Login</button><div id="loginErr" class="error"></div><p class="small muted">Customer? <a href="?view=customer">Open customer portal</a></p></div></div><div class="footer"><b>${esc(s().company_name||DEFAULT_COMPANY_NAME)}</b> • ${esc(s().agency_name||"GANTALAMMA MILK AGENCY")}</div>`;
}
...
function customerLogin(){
 app.innerHTML=`<div class="top"><div class="wrap">${companyBrand()}<div class="sub portal-sub">Customer Portal</div></div></div><div class="wrap login"><div class="card">
 ...
 </div></div><div class="footer"><b>${esc(s().company_name||DEFAULT_COMPANY_NAME)}</b> • ${esc(s().agency_name||"GANTALAMMA MILK AGENCY")}</div>`;
}
